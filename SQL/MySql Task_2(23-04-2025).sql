...
SELECT SUM(quantity * price) As total_sales_value from sales_data;

---- Monthly Sales Trends----
### Show total sales value by month:=
SELECT 
    date_format(order_date, "%Y-%M") AS Month,
    SUM(quantity * price) AS total_sales from sales_data
    GROUP BY MONTH
    ORDER BY MONTH;
...
SELECT 
     Product_name,
     SUM(quantity * price) As total_revenue from sales_data
...
SELECT 
     customer_id,
     COUNT(*) AS order_count from sales_data
...
SELECT 
     category,
     ROUND(SUM(quantity * price) / ( SELECT SUM(quantity * price ) from sales_data) * 100,2) AS revenue_percentage 
     from sales_data
     GROUP BY category
...
SELECT
    COUNT(*) AS customer_with_multiple_purchase
    FROM (
		   SELECT 
